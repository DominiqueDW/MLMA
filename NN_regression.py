...
def import_data(neigh, norm, stand, clean, test, optimise_NN, params):
    if clean or optimise_NN:
        if test:
...
def import_model(neigh, norm, stand):
    ...
    filename = "Resources/Model_data/NN_results/Models/" + \
        str(neigh) + "/" + model_name
    filename2 = "Resources/Model_data/NN_results/Models/" + \
        str(neigh) + "/scaler.pkl"

    NN_model = tf.keras.models.load_model(filename)
    sc_y = pickle.load(open(filename2, "rb"))

    return NN_model, sc_y
...
def compile_model(NN_model, loss_func, opt_func, lr, momentum):
    ...
    if loss_func == "MAE":
        ...
        if opt_func == "SGD":
            ...
            NN_model.compile(loss=tf.keras.losses.mse, optimizer=tf.keras.optimizers.RMSprop(learning_rate=lr),
                             metrics=["mse"])
...
def fit_model(NN_model, X_train, y_train, X_test, y_test, epochs, optimise_NN):
    if optimise_NN:
        ...
        callback = CustomEarlyStopping(monitor='val_loss', patience=5, verbose=0, restore_best_weights=True)
...
def plot_predictions(y_test, y_pred, neigh, save_plt_pred, test):
    ...
    if save_plt_pred:
        if test:
            filename = str(neigh) + "_NN_test.png"
        else:
            filename = str(neigh) + "_NN_train.png"

        data_path = r''+os.path.join(os.path.dirname(__file__),
                                     'Resources/Model_data/NN_results/Models', neigh, filename)
...
def save_predictions(y_test, y_pred, neigh, stand, norm, test, save_fusion_pred):
    df = pd.DataFrame({'Predicted value': y_pred, 'Real value': y_test,
                      'Percentage error': ((abs(y_pred-y_test))/y_test*100)})
    ...
    if test:
        filename2 = str(neigh) + "_test_NN.xlsx"
        data_path = r''+os.path.join(os.path.dirname(__file__),
                                     'Resources/Model_data/NN_results/Test_predictions', filename)
        data_path2 = r''+os.path.join(os.path.dirname(__file__),
                                      'Resources/Model_data/Fusion_results/Data/Test', neigh, filename2)
    else:
        filename2 = str(neigh) + "_NN.xlsx"
        data_path = r''+os.path.join(os.path.dirname(__file__),
                                     'Resources/Model_data/NN_results/Validation_predictions', filename)
        data_path2 = r''+os.path.join(os.path.dirname(__file__),
...
def select_opt_NN(neigh):
    filename = "Resources/Model_data/NN_results/Optimisation_tests/" + \
...
def main():
    ...
    if optimise_NN:
        ...
        neigh_df, Y_label = import_data(
            neighbourhoods[neigh_num], clean, norm, stand, test, optimise_NN, params)
    ...
    if train_NN:
        ...
        history = fit_model(
            NN_model, X_train, y_train, None, None, epochs, optimise_NN)
        ...
        if save_graph_model:
            ...
            file_name = "Resources/Model_data/NN_results/Graphical_models/" + model_name
        ...
        if save_model:
            ...
            filename2 = "Resources/Model_data/NN_results/Models/" + \
                str(neighbourhoods[neigh_num]) + "/scaler.pkl"
        ...
        if plt_pred:
            plot_predictions(y_train, y_pred, neighbourhoods[neigh_num], save_plt_pred, test)
    ...
    if vali_para:
        ...
        NN_model, sc_y = import_model(neighbourhoods[neigh_num], norm, stand)
        ...
        if save_pred:
            ...
            save_predictions(
                y_train, y_pred, neighbourhoods[neigh_num], stand, norm, test, save_fusion_pred)
    ...
    if test:
        ...
        if save_pred:
            ...
            save_predictions(
                y_test, y_pred_test, neighbourhoods[neigh_num], stand, norm, test, save_fusion_pred)
    ...
    if plt_NN_diagram:
        # Block visualization
        plot_model(NN_model, to_file="NN_models\model_1.png", show_shapes=True)
